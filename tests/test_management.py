...
def _patch_delinea_session(monkeypatch, token="tok", base_url="https://ss.example.com"):
    """Replace server.delinea and tools.delinea with a SimpleNamespace stub."""
    from delinea_mcp import tools

    stub = SimpleNamespace(token=token, base_url=base_url)
    monkeypatch.setattr(server, "delinea", stub)
    monkeypatch.setattr(tools, "delinea", stub)


def test_set_secret_field_env_bash_stdin(monkeypatch):
    _patch_delinea_session(
...
def test_set_secret_field_env_bash_env_source(monkeypatch):
    _patch_delinea_session(monkeypatch)
    script = server.set_secret_field_environment_variable(
...
def test_update_secret_generated_password_success(monkeypatch):
    ...
    assert "password" not in str(result["result"].get("value", ""))
    assert result["verification"]["id"] == 42
