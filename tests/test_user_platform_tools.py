...
def test_platform_user_functions(monkeypatch):
    ...
    assert res == {"result": {"ok": True}, "verification": {"ok": True}}
    res = user_platform_tools.platform_user_management("update", user_id="id", data={"a": 1})
    assert res == {"result": {"ok": True}, "verification": {"ok": True}}
...
def test_create_user_nonjson(monkeypatch):
    ...
    class BadResponse(DummyResponse):
        def json(self):
            raise ValueError
    monkeypatch.setattr(user_platform_tools.requests, "post", lambda *a, **k: BadResponse())
    res = user_platform_tools.platform_user_management("create", data={"Name": "u"})
...
def test_other_calls_nonjson(monkeypatch):
    ...
    assert user_platform_tools.platform_user_management("update", user_id="x", data={}) == {
        "result": {"error": "tx"},
        "verification": {"error": "tx"},
    }
    assert user_platform_tools.platform_user_management("search", username="x") == {"error": "tx"}
