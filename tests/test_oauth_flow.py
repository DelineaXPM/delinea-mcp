...
def setup_server(monkeypatch):
    ...
    mount_sse_routes(app, server.mcp, require_scopes(["mcp.read"], audience="http://testserver"))
    return app
...
async def test_full_oauth_flow(monkeypatch):
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        r = await client.get("/oauth/authorize", params={"client_id": cid, "redirect_uri": "http://testserver/cb", "scope": "mcp.read"})
        ...
        r = await client.post("/oauth/token", data={"grant_type": "authorization_code", "code": code, "client_id": cid, "client_secret": cs})
