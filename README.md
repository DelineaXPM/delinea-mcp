...
<<<<<<< HEAD
- **ssl_keyfile** - Path to the SSL key for HTTPS (e.g. `privkey.pem`).
- **ssl_certfile** - Path to the SSL certificate for HTTPS (e.g. `fullchain.pem`).
...
- **jwt_key_path** - Location of the RSA key pair used for OAuth tokens. Defaults to `.cache/jwt.json`; autogenerated if it doesn't exist.
- **oauth_db_path** - Path to the OAuth database file. Defaults to `.cache/oauth.db`; autogenerated if it doesn't exist.
- **enabled_tools** - List of tool names to register. An empty list enables all tools. It is highly recommended to enable tools selectively per use case or task. See the `docs/` folder for examples.
