...
class DelineaSession:
    ...
    * **Password grant** (default) — authenticates directly against the Secret
      Server ``/oauth2/token`` endpoint using a username and password.
    * **Platform client credentials** — authenticates against the Delinea
      Platform identity endpoint using a client ID and secret, then uses the
      resulting token to call Secret Server APIs.  Activated by setting
      *platform_hostname* (and optionally via the ``PLATFORM_HOSTNAME``
    ...
    def authenticate(
        ...
        """Authenticate via password grant and store bearer token.
